...
import com.kelan.core.constant.WebConstants;
import com.kelan.core.session.cache.SessionCache;
import com.kelan.core.session.id.SessionIdGenerator;
import com.kelan.core.util.RequestUtils;
...
public class CacheSessionProvider implements SessionProvider, InitializingBean {
	...
	public String getSessionId(HttpServletRequest request,
		...
		if (root == null || root.length() != 32 || !sessionCache.exist(root)) {
			...
			sessionCache.setSession(root, new HashMap<String, Object>(),
					sessionTimeout);
			response.addCookie(createCookie(request, root));
		}
		request.setAttribute(CURRENT_SESSION_ID, root);
	...
	private Cookie createCookie(HttpServletRequest request, String value) {
		Cookie cookie = new Cookie(WebConstants.SESSION_COOKIE, value);
